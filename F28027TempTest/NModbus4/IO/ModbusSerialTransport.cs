namespace Modbus.IO
{
    ...
    public abstract class ModbusSerialTransport : ModbusTransport
    {
        ...
        public delegate void TransmitDataHandler(byte[] str);
        internal event TransmitDataHandler DataTransmited;

        public delegate void ReceiveDataHandler(byte[] str);
        internal event ReceiveDataHandler DataReceived;

        protected void RaiseTransmitDataHandler(byte[] str)
        {
            DataTransmited?.Invoke(str);
        }
        protected void RaiseReceiveDataHandler(byte[] str)
        {
